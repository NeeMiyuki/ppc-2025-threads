...
void shulpin_tbb_test_module::MainTestBody(std::vector<shulpin_i_jarvis_tbb::Point> &input,
  ...
  shulpin_i_jarvis_tbb::JarvisSTLParallel stl_task(task_data_par);
  ASSERT_EQ(stl_task.Validation(), true);
  stl_task.PreProcessing();
  stl_task.Run();
  stl_task.PostProcessing();

  shulpin_tbb_test_module::VerifyResults(result_seq, result_tbb);
}
...
void shulpin_tbb_test_module::RandomTestBody(std::vector<shulpin_i_jarvis_tbb::Point> &input,
  ...
  shulpin_tbb_test_module::VerifyResults(expected, result_tbb);
}
